...
function ListeSpecialites({ refreshKey, filieresMap }) {
    ...
    const fetchAll = async () => {
        ...
        setError(null);
        try {
            const res = await SpecialiteService.getAll();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await SpecialiteService.remove(id);
            fetchAll();
        ...
        <div>
          <h4 className="fw-bold mb-1 text-dark d-flex align-items-center gap-2">
            <div className="d-flex align-items-center justify-content-center rounded-3"
              ...
              <i className="bi bi-bookmark-star-fill text-primary"></i>
            </div>
            Liste des spécialités enregistrées
          </h4>
          <p className="text-muted mb-0 ms-1">Gestion des spécialités</p>
        </div>
      ...
      {!loading && !error && items.length === 0 && (
        <div className="text-center py-5">
          <div className="mx-auto mb-4 d-flex align-items-center justify-content-center rounded-circle"
            ...
            <i className="bi bi-bookmark-star-fill fs-1 text-secondary"></i>
          </div>
          <h5 className="fw-bold text-dark">Aucune spécialité enregistrée</h5>
          <p className="text-muted mb-0">Ajoutez une nouvelle spécialité pour commencer.</p>
        </div>
      )}
                ...
                <tr key={s.id}
                  ...
                  <td className="py-3">
                    <span className="badge text-dark px-3 py-2 rounded-pill shadow-sm">
                      {filieresMap?.[s.filieres_id] ?? s.filieres_id}
                    </span>
                  </td>
                  <td className="py-3">
...
export default ListeSpecialites;
