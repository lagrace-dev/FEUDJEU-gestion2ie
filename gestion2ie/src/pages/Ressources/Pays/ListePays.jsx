...
function ListePays({ refreshKey }) {
    ...
    const fetchAll = async () => {
        ...
        setError(null);
        try {
            const res = await PaysService.getAll();
  ...
  <div className="card-body p-4">
    ...
    {!loading && !error && items.length === 0 && (
      <div className="text-center py-5">
        ...
        <h5 className="fw-bold text-dark">Aucun pays enregistré</h5>
        <p className="text-muted mb-0">Ajoutez un nouveau pays pour commencer.</p>
      </div>
    )}
    ...
    {!loading && !error && items.length > 0 && (
      <div className="table-responsive">
        <table className="table align-middle mb-0"
            ...
            <tr>
             <th className="border-0 text-uppercase  fw-semibold px-3 text-white"
              ...
              <th className="border-0 text-uppercase fw-semibold text-white"
                                style={{ fontSize: "0.8rem" }}> Nationalité
                            </th>
              <th className="border-0 text-uppercase fw-semibold text-white"
                                style={{ fontSize: "0.8rem" }}> Code
                            </th>
              <th className="border-0 text-uppercase fw-semibold text-white"
                                style={{ fontSize: "0.8rem" }}> ISO
                            </th>
          ...
          <tbody>
            {items.map((p) => (
              <tr key={p.id} style={{background: "#f8fafc", borderRadius: "16px", transition: "0.3s ease",}}
        className="shadow-sm">
            ...
            <span className="px-3 py-2 rounded-pill fw-semibold"
                style={{background:"rgba(16, 185, 129, 0.12)",
                        color: "#059669",
                        fontSize: "0.9rem",}}>{p.nationalite}</span>
                </td>
                <td className="py-3">
