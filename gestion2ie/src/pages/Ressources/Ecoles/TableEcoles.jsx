...
function TableEcoles({ refreshKey }) {
    ...
    const fetchAll = async () => {
        ...
        setError(null);
        try {
            const res = await EcoleService.getAll();
    ...
    return (
        <div className="card border-0 rounded-4 overflow-hidden"
            ...
            style={{   width: "45px",height: "45px",background: "rgba(37, 99, 235, 0.12)",}}>
                                <i className="bi bi-building text-primary"></i>
                            </div>
                            Liste des écoles enregistrées
                        </h4>
    <p className="text-muted mb-0 ms-1">Gestion des écoles et établissements</p>
                    </div>

        <div className="px-3 py-2 rounded-pill fw-semibold" 
        style={{background: "rgba(37, 99, 235, 0.1)",color: "#2563eb",fontSize: "0.9rem",}}>
                        {items.length} école{items.length > 1 ? "s" : ""}
                    </div>
    </div>
    {loading && (<div className="text-center py-5">
            <div className="spinner-border text-primary mb-3" role="status"></div>
            <p className="text-muted">Chargement des écoles...</p></div> )}
    {error && (<div className="alert alert-danger border-0 shadow-sm rounded-3 d-flex align-items-center">
            <i className="bi bi-exclamation-triangle-fill me-2"></i>{error} </div>)}
    {!loading && !error && items.length === 0 && (
        ...
        <i className="bi bi-building fs-1 text-secondary"></i>
            ...
            <h5 className="fw-bold text-dark">Aucune école enregistrée</h5>
                <p className="text-muted mb-0">Ajoutez une nouvelle école pour commencer.</p>
                    </div>
                )}                   
    ...
    <table className="table align-middle mb-0"
         ...
         <thead className="table-dark text-uppercase">                   
            ...
            <th className="border-0 text-uppercase fw-semibold px-3 text-white"
                style={{ fontSize: "0.8rem" }}> Libellé</th>
            ...
            <th className="border-0 text-uppercase fw-semibold text-white"
                style={{ fontSize: "0.8rem" }}>Adresse</th>
            ...
            <tbody>
                {items.map((e) => (<tr key={e.id} style={{background: "#f8fafc",borderRadius: "16px",transition: "0.3s ease",}}
                          className="shadow-sm">           
                    <td className="border-0 px-3 py-3 fw-semibold text-dark"
                     ...
                     <div className="d-flex align-items-center gap-3">                            
                        <div className="d-flex align-items-center justify-content-center rounded-3"                        
                            style={{width: "42px",height: "42px",  background: "linear-gradient(135deg, rgba(37,99,235,0.15), rgba(59,130,246,0.08))",          
                            }}>
                ...
                <td className="border-0 py-3">
                 <span className="px-3 py-2 rounded-pill fw-semibold"
                    style={{ background: "rgba(59, 130, 246, 0.12)", color: "#1e40af",fontSize: "0.9rem",}}>   
                {e.email || "-"} </span></td>
                                        
        <td className="border-0 py-3">
        <span className="badge px-3 py-2 rounded-pill shadow-sm" style={{background: "rgba(100, 116, 139, 0.1)",color: "#475569",}}>
            {e.adresse || "-"}</span></td>
    <td className="border-0 py-3">
    ...
    <td className="border-0 text-end px-4 py-3"
        ...
        <button className="btn btn-sm btn-outline-danger rounded-pill px-3 shadow-sm" onClick={() => handleDelete(e.id)}>
    <i className="bi bi-trash me-1"></i> Supprimer</button></td>
    </tr>
        ))}
        </tbody>
        </table>
