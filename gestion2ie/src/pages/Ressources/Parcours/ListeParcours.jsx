...
function ListeParcours({ refreshKey, specialitesMap, niveauxMap, cyclesMap }) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await ParcoursService.remove(id);
            fetchAll();
    ...
    <div className="card-body p-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted mb-0 ms-1">Gestion des parcours</p>
        </div>
      ...
      {!loading && !error && items.length === 0 && (
        <div className="text-center py-5">
          <div className="mx-auto mb-4 d-flex align-items-center justify-content-center rounded-circle"
            ...
            <i className="bi bi-map-fill fs-1 text-secondary"></i>
          </div>
          <h5 className="fw-bold text-dark">Aucun parcours enregistré</h5>
          <p className="text-muted mb-0">Ajoutez un nouveau parcours pour commencer.</p>
        </div>
      )}
                ...
                <tr key={p.id}
                  ...
                  <td className="py-3">
                    <span className="badge text-dark px-3 py-2 rounded-pill shadow-sm">
                      {specialitesMap?.[p.specialites_id] ?? p.specialites_id}
                    </span>
                  </td>
                  <td className="py-3">
                    <span className="badge text-secondary px-3 py-2 rounded-pill shadow-sm">
                      {niveauxMap?.[p.niveaux_id] ?? p.niveaux_id}
                    </span>
                  </td>
                  <td className="py-3">
                    <span className="badge text-success px-3 py-2 rounded-pill shadow-sm">
                      {p.cycles_id ? (cyclesMap?.[p.cycles_id] ?? p.cycles_id) : ""}
                    </span>
                  </td>
