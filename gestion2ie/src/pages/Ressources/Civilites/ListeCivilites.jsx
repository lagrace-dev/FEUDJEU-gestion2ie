...
function ListeCivilites({ refreshKey }) {
    ...
    const fetchAll = async () => {
        ...
        setError(null);
        try {
            const res = await CiviliteService.getAll();
            setItems(Array.isArray(res.data) ? res.data : []);
        } catch (err) {
            setError("Impossible de charger les civilités.");
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("Supprimer cette civilité ?")) return;
        try {
            await CiviliteService.remove(id);
            fetchAll();
    ...
    <div className="card-body p-4">
      ...
      {loading && (
        <div className="text-center py-5">
          ...
          <p className="text-muted">Chargement des civilités...</p>
        </div>
      )}
      ...
      {!loading && !error && items.length === 0 && (
        <div className="text-center py-5">
          <div className="mx-auto mb-4 d-flex align-items-center justify-content-center rounded-circle"
            ...
            <i className="bi bi-person-badge-fill fs-1 text-secondary"></i>
          </div>
          <h5 className="fw-bold text-dark">Aucune civilité enregistrée</h5>
          <p className="text-muted mb-0">Ajoutez une nouvelle civilité pour commencer.</p>
        </div>
      )}
              ...
              {items.map((c) => (
                <tr key={c.id}
                  ...
                  <td className="py-3">
                    ...
                    style={{background: "rgba(83, 227, 126, 0.79)", color: "#636781", fontSize: "0.9rem"}}>
                      {c.abreviation}
                    </span>
                  </td>
